...
По умолчанию скрипт также проверяет локальный CPE-индекс. Если активная CPE-генерация уже есть в SQLite, она переиспользуется без повторного скачивания больших архивов. Если CPE-индекса нет, будут скачаны официальные `nvdcpe-2.0.tar.gz` и `nvdcpematch-2.0.tar.gz`, после чего появится статистика `CPE Dictionary`, `CPE Match` и `Active CPE generation`. Для аварийного обновления только CVE/CISA можно временно отключить CPE:
