...
7. Пользователь явно выбирает источник уязвимостей: автоматический приоритет локальной БД, только локальная БД/кэш или ограниченный онлайн-профиль CISA/NVD/ФСТЭК.
...
- переключатель «Источник уязвимостей» выбирает `Авто`, `Только локальная база` или `Только онлайн`;
...
Для отзывчивости окна живая таблица публикует не более 400 событий пакетов за запуск и обрабатывает очередь порциями до 64 событий. Весь поток `tshark` при этом читается и анализируется; лимит относится только к оперативной отрисовке.
...
| Источник | Что делает | Когда использовать |
|---|---|---|
| Авто: локальная -> онлайн | Сначала ищет валидную `vulnerability_sources.db`; если БД отсутствует, использует кэш и ограниченный онлайн-профиль | Рекомендуемый режим по умолчанию |
| Только локальная база | Запрещает HTTP/curl для корреляции и использует только SQLite-базу и локальный кэш; отсутствие данных фиксируется как `insufficient_data` | Закрытый контур, повторяемый аудит, максимальная приватность |
| Только онлайн | Игнорирует локальную SQLite-базу и выполняет ограниченную проверку официальных CISA/NVD/ФСТЭК с сохранением snapshots в кэш | Диагностика источников или быстрая проверка без подготовленной БД |

Онлайн-профиль намеренно ограничен: до `6` групповых запросов NVD, `1` поискового запроса ФСТЭК, `1` страницы и `2` карточек ФСТЭК. Тайм-аут одного CISA/NVD-запроса составляет `10` секунд, ФСТЭК - `6` секунд. В интерфейсе показываются текущий источник и счётчик `x/y`; при достижении бюджета аудит продолжается, а в отчёт добавляется диагностическое предупреждение. Это исключает многочасовой цикл онлайн-карточек.

Для полного охвата каталога используйте «Обновить базы» и режим `Авто` или `Только локальная база`. При наличии валидной `vulnerability_sources.db` локальная CPE/NVD/ФСТЭК-корреляция выполняется без указанного лимита и без повторного онлайн-поиска ФСТЭК.
