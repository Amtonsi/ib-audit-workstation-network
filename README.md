...
| `Порты Nmap` | `1-65535` | проверяется заданный пользователем диапазон портов; без дополнительных ключей Nmap выполняет TCP-проверку, UDP включается только явно через дополнительные аргументы |
