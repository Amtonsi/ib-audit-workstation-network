...
5. Если включён сетевой модуль, пользователь открывает окно «Команды сети» и выбирает профиль Nmap/tshark: какие ключи включить, какие отключить, какие дополнительные аргументы и фильтры применить.
6. Nmap собирает открытые сервисы, а tshark фиксирует сетевые потоки за выбранный интервал. Оба инструмента запускаются только при явном включении сетевой проверки.
...
- блок «Сетевая проверка» содержит главный переключатель Nmap, переключатель захвата трафика и кнопку «Команды сети»;
- «Команды сети» открывает окно с чекбоксами для ключей Nmap/tshark, полями дополнительных аргументов, фильтром захвата и русскими подсказками при наведении на каждую команду;
...
|---|---:|---|
| `Включить Nmap` | выключено | активное сканирование сети не запускается, пока пользователь явно не поставит галочку |
| `Захват трафика` | выключено | `tshark` не запускается, пока пользователь явно не включит захват |
| `Цели Nmap` | пусто | если поле пустое, приложение пытается определить локальные IPv4-сети; безопаснее явно указывать разрешённый диапазон |
...
| `Профиль скорости` | `T2` | осторожный режим Nmap; снижает агрессивность проверки по сравнению с быстрыми профилями |
| `Доп. аргументы Nmap` | пусто | дополнительные ключи не добавляются, пока пользователь явно их не укажет |
| `Интерфейс tshark` | пусто | приложение пытается выбрать первый доступный интерфейс; для точности лучше указать номер из `tshark -D` |
...
| `Определять ОС хоста` | `-O` | включено | Nmap пытается определить ОС удалённого узла; может требовать прав администратора и увеличивать время проверки |
...
```powershell
nmap [-n] [-Pn] [-T2] [-open] -oX - -p <ports> [-sV] [-O] <extra_args> <targets>
```
...
Приложение использует не графический Wireshark, а консольный `tshark`, который устанавливается вместе с Wireshark.
...
Используйте готовый `.spec`, потому что он добавляет rulepack-файлы и локальные runtime-каталоги `tools/nmap`, `tools/wireshark` и `tools/npcap` в автономную сборку.
